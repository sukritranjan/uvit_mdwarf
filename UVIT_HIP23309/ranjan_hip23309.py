...
if compare_hst_uvit_hip23309_data:
   ...
   ###Calculate "chi-square". Approximate HST data as perfect (no error)
    ...
    total_fuv_uvit=np.sum(uvit_wav_deltas_cut*uvit_flux_cut)
    total_fuv_uvit_err=np.sqrt(np.sum((uvit_wav_deltas_cut*uvit_fluxerr_cut)**2.0))
    ...
    floor_est=(total_fuv_uvit-total_fuv_hst)/(1700-1300)
    
    #Offset fit. 
    def functhis(x, floor):
    ...
    # #linear fit. 
    # def functhis2(x, floor, slope):
    #     return floor+hip23309_hst_smoothed_uvitbinned + slope * x
    
    # guess=np.array([floor_est, 0.0])
    # popt2, pcov2=scipy.optimize.curve_fit(functhis2, uvit_wav_centers_cut, uvit_flux_cut, p0=guess, sigma=uvit_fluxerr_cut, method='lm')
    
    
    # chisquare3=np.sum((functhis2(uvit_wav_centers_cut, popt2[0], popt2[1])-uvit_flux_cut)**2.0/(uvit_fluxerr_cut)**2.0)
    # dof3=len(uvit_wav_centers_cut)-2
    # rchisquare3=chisquare3/dof3  
    # print('"Chi-square w/linear term": {0:3.1f}, dof:{1:3f} "reduced "chi square" w/linear term": {2:1.2f}'.format(chisquare3, dof3, rchisquare3))
